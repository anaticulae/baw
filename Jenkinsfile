...
pipeline{
    ...
    // image '169.254.149.20:6001/arch_python_git_baw:v1.68.0'
    stages{
        stage('integrate'){
        ...
        stage('others'){
            when{branch 'develop'}
            parallel{
                stage('utilo'){steps{build job: 'caelum/utilo/integrate'}}
                stage('utilotest'){steps{build job: 'caelum/utilotest/integrate'}}
